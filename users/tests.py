...
class FriendshipTestCase(TestCase):
    ...
    def test_split_amount_view(self):
        ...
        response = self.c.post(reverse('debt_split_amount'),{'item':'','amount': 10, 'paid_user': self.friend, 'friends': []})
        self.assertEqual(response.status_code, 200)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': None, 'paid_user': self.friend, 'friends': []})
        self.assertEqual(response.status_code, 200)
        ...
        transactions= Friendship.objects.get(user=friend, friend=self.user).transactions.all()
        ...
        transactions= Friendship.objects.get(user=self.user, friend=friend).transactions.all()
        self.assertEqual(len(transactions),1)
        self.assertEqual(transactions[0].amount,100)        
        self.assertEqual(transactions[0].owe_id,friend.id)
        self.assertEqual(transactions[0].item,'testitem1')
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 12, 'paid_user': self.user.username, 'friends': [self.user.username, friend.username, new_friend.username]})
        transactions = Friendship.objects.get(user=self.user, friend=new_friend).transactions.all()
        self.assertEqual(len(transactions),1)
        ...
        transactions = Friendship.objects.get(user=new_friend, friend=self.user).transactions.all()
        self.assertEqual(len(transactions),1)
        self.assertEqual(transactions[0].amount,4)        
        self.assertEqual(transactions[0].owe_id, new_friend.id)
        self.assertEqual(transactions[0].item,'testitem')
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item':'item', 'amount':20, 'paid_user': self.user.username, 'friends': [self.user.username, other_friend.username]})
        self.assertEqual(response.status_code,302)
        transactions = Friendship.objects.get(user=self.user, friend=other_friend).transactions.all()
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 100, 'paid_user': self.user.username, 'friends': [self.user.username, friend.username]})
        self.assertEqual(response.status_code, 302)
        friendship = Friendship.objects.get(user=self.user, friend=friend)
        reverse_friendsip = Friendship.objects.get(user=friend, friend=self.user)
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 500, 'paid_user': friend.username, 'friends': [self.user.username, friend.username]})
        self.assertEqual(response.status_code, 302)
        friendship = Friendship.objects.get(user=self.user, friend=friend)
        reverse_friendsip = Friendship.objects.get(user=friend, friend=self.user)
        ...
        self.assertEqual(friendship.net_amount, 96)
        self.assertEqual(friendship.owe, self.user.id)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 8, 'paid_user': friend.username, 'friends': [self.user.username, friend.username]})
        friendship = Friendship.objects.get(user=self.user, friend=friend)
        reverse_friendship = Friendship.objects.get(user=friend, friend=self.user)
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item': 'item','amount': 50, 'paid_user':  self.user.username, 'friends': [self.user.username, friend.username]})
        friendship = Friendship.objects.get(user=self.user, friend=friend)
        reverse_friendship = Friendship.objects.get(user=friend, friend=self.user)
        ...
        response = self.c.post(reverse('debt_split_amount'), {'item':'item', 'amount': 60, 'paid_user': new_friend.username, 'friends': [self.user.username, new_friend.username, friend.username]})
        self.assertEqual(response.status_code, 302)
        friendship_1 = Friendship.objects.get(user=self.user, friend=new_friend)
        self.assertEqual(friendship_1.owe, self.user.id)
        self.assertEqual(friendship_1.net_amount, 6)
        reverse_friendship_1 = Friendship.objects.get(user=new_friend, friend=self.user)
        self.assertEqual(reverse_friendship_1.owe, self.user.id)
        self.assertEqual(reverse_friendship_1.net_amount, 6)
        friendship_2 = Friendship.objects.get(user=new_friend, friend=friend)
        reverse_friendship_2 = Friendship.objects.get(user=friend, friend=new_friend)
        ...
        reverse_friendship_3 = Friendship.objects.get(friend=friend, user=self.user)
