...
class FriendshipTestCase(TestCase):
    ...
    def test_split_amount_view(self):
        ...
        self.assertEqual(transactions[0].owe_id,self.user.id)
        ...
        new_friend = User.objects.get(username__exact='test0')
        ...
        self.assertEqual(transactions[1].amount,4)        
        self.assertEqual(transactions[1].owe_id,self.user.id)
        self.assertEqual(transactions[1].item,'testitem')
        ...
        self.assertEqual(transactions[2].amount,10)        
        self.assertEqual(transactions[2].owe_id,self.user.id)
        self.assertEqual(transactions[2].item,'item')
        transactions = Friendship.objects.get(user=other_friend, friend=self.user).transactions.all()
        self.assertEqual(len(transactions),3)
        self.assertEqual(response.status_code, 302)
        friend = User.objects.get(username='test0')
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 100, 'paid_user': self.user.username, 'friends': [self.user, friend]})
        self.assertEqual(response.status_code, 302)
        ...
        self.assertEqual(friendship.net_amount, 164)
        self.assertEqual(friendship.owe, friend.id)
        self.assertEqual(reverse_friendsip.net_amount, 164)
        self.assertEqual(reverse_friendsip.owe, friend.id)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 500, 'paid_user': friend, 'friends': [self.user.username, friend.username]})
        self.assertEqual(response.status_code, 302)
        friendship = Friendship.objects.get(user=self.user, friend=friend)
        reverse_friendsip = Friendship.objects.get(user=friend, friend=self.user)
        self.assertEqual(friendship.net_amount, 86)
        self.assertEqual(friendship.owe,self.user.id)
        self.assertEqual(friendship.net_amount, 86)
        self.assertEqual(friendship.owe, self.user.id)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 28, 'paid_user': friend.username, 'friends': [self.user, friend]})
        friendship = Friendship.objects.get(user=self.user, friend=friend)
