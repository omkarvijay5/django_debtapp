...
class FriendshipTestCase(TestCase):
    ...
    def test_split_amount_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 10, 'paid_user': self.friend, 'friends': None})
        self.assertEqual(response.status_code, 200)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem', 'amount': 10, 'paid_user': None, 'friends': self.user.username})
        self.assertEqual(response.status_code, 200)
        response = self.c.post(reverse('debt_split_amount'), {'item': 'testitem1', 'amount': 100, 'paid_user': self.user, 'friends': self.user.username})
