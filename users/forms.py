...
class SplitBillForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['paid_user'] = forms.ChoiceField(
            choices=get_users(self.user)
        )
    ...
    item = forms.CharField(label="For What?",
                           widget=forms.TextInput(
                               attrs={'placeholder': "ex: burger and petrol"}))
    amount = forms.IntegerField(label="How much?",
                                widget=forms.TextInput(
                                    attrs={'placeholder': "ex: 100 and 500"}))
        ...
        if friends:
            if len(friends) == 1 and paid_user == user and user in friends:
                raise forms.ValidationError(
                    'You cannot add bill to yourself.\
